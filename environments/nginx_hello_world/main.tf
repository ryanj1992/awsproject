...
module "ecs" {
  ...
  execution_role_arn = module.endpoints[each.key].role_arn
...
module "endpoints" { # needs updating with each.key
  ...
  security_group    = module.networking[each.key].private_sg_id
  vpc_id            = module.networking[each.key].main_vpc_id
  private_subnet_id = module.networking[each.key].private_subnet_id
