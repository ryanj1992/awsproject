...
resource "aws_vpc_endpoint" "ecs_api" {
  vpc_id            = var.vpc_id
  service_name      = "com.amazonaws.us-east-1.ecr.api" # needs replacing with var.environment
...
resource "aws_vpc_endpoint" "ecs_dkr" {
  vpc_id            = var.vpc_id
  service_name      = "com.amazonaws.us-east-1.ecr.dkr" # needs replacing with var.environment
  vpc_endpoint_type = "Interface"
...
resource "aws_vpc_endpoint" "s3" {
  vpc_id       = var.vpc_id
  service_name = "com.amazonaws.us-east-1.s3" # needs replacing with var.environment
